...
exports.allimg_plate_no = async (req, res) => {
  const { id } = req.body;
  console.log("hey i am id:", id);
  if (id) {
    try {
      let sql = `SELECT * FROM allinfo WHERE id = '${id}'`;
      ...
      if (result) {
        resSend(res, true, 200, "All allimg_plate_no", result, null);
      } else {
        resSend(res, false, 200, "No Record Found!", result, null);
      }
...
//  licence image api allimg_plate_no
exports.allinfoimg_plateimg = async (req, res) => {
  const { id } = req.body;
  console.log("hey i am id:", id);
  if (id) {
    try {
      let sql = `SELECT * FROM allinfo `;
